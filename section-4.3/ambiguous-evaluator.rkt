...
(define (cond-capturing-clause? clause)
...
(define (or->if exp)
  (define (transform exp)
    (cond ((null? exp) 'false)
          ((null? (cdr exp)) (car exp))
          (else (make-if (car exp) (car exp) (transform (cdr exp))))))
  (transform (or-expressions exp)))
...
(define (analyze-or exp)
  (define (analyze-procs procs)
    (cond ((null? procs) (lambda (env) false))
          ((null? (cdr procs)) (lambda (env) ((car procs) env)))
          (else (let ((rproc (analyze-procs (cdr procs))))
                  (lambda (env) (or ((car procs) env) (rproc env)))))))
  (analyze-procs (map analyze (or-expressions exp))))
...
(define (let->combination exp)
  (let* ((clauses (let-clauses exp))
         (variables (map let-variable clauses))
