...
(define (eval exp env)
  (cond ((self-evaluating? exp) exp)
        ...
        ((continue? exp) (eval-continue exp env))
        ((application? exp)
...
(define (setup-environment)
  (let ((initial-env
    ...
    (define-variable! 'loop-state 'not-running initial-env)
    initial-env))
...
(define (loop-state env)
  (lookup-variable-value 'loop-state env))

(define (set-loop-state! value env)
  (set-variable-value! 'loop-state value env))

(define (eval-while exp env)
  (let ((env (extend-environment (list 'loop-state) (list 'running) env))
        (predicate (while-predicate exp))
        (body (while-body exp)))
    (define (stop?)
      (eq? (loop-state env) 'break))
    (define (keep-going?)
      (true? (eval predicate env)))
    (define (run)
      (cond ((stop?) true)
            ((keep-going?)
             (set-loop-state! 'running env)
...
(define (eval-break exp env)
  (if (eq? (loop-state env) 'not-running)
      (error "Can't break while loop is not running")
      (set-loop-state! 'break env)))

(define (continue? exp) (tagged-list? exp 'continue))
(define (eval-continue exp env)
  (if (eq? (loop-state env) 'not-running)
      (error "Can't continue while loop is not running")
      (set-loop-state! 'continue env)))

(define (short-circuit? env)
  (let ((value (loop-state env)))
    (or (eq? value 'break) (eq? value 'continue))))
